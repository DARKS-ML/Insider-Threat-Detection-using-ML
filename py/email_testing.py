...
import pickle
model = pickle.load(open('../pkl/email_result.pkl', 'rb'))
email_ascore = model.decision_function(email_user_matrix)
threat = model.predict(email_user_matrix)
...
df_email_result['user'] = df_email_user_stats_sec['user_id']
df_email_result['ascore'] = email_ascore
df_email_result['threat'] = threat
df_email_result.to_csv('../output/email_result.csv', index=False)
